...
def get_model_count(config: DataObjects.PipelineConfig):
    return 7 if config.control_net.name is not None else 6
...
def generate(
    ...
    # Prompt Cache
    prompt_cache_key = (options.prompt, DEFAULT_NEGATIVE_PROMPT, options.guidance_scale > 1.0)
    ...
    # Pipeline Options
    system_prompt = T2V_DEFAULT_SYSTEM_PROMPT if _processType == ProcessType.TextToVideo else I2V_DEFAULT_SYSTEM_PROMPT
